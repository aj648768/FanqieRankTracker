...
> 📚 追踪**番茄小说男频/女频阅读榜与新书榜**，每日自动抓取四个榜单并结合 AI 生成趋势分析，部署为在线看板。
...
| 功能 | 说明 |
|------|------|
| 🕷️ 自动爬取 | 每日定时抓取番茄男频/女频的阅读榜、新书榜 Top 30 |
| 📊 趋势对比 | 自动对比相邻两天数据：新上榜 / 掉榜 / 排名变化 / 阅读量增长 |
| 🤖 AI 风向分析 | 接入 OpenAI 兼容 API，按分类生成市场趋势速评 |
| 🧭 类型风向标 | 独立趋势页按榜单聚合多日数据，用 AI 总结综合赛道、具体热门分类和高频题材；未配置 API 时自动规则兜底 |
...
看板可在四个榜单间切换；右上角的 **风向标** 可进入 `trend.html`，先查看当下火热综合赛道、具体热门分类和高频题材，再按榜单与具体类型查看近 7 / 14 / 30 日或全部周期的趋势分析。全站热点会优先使用 AI 总结，未配置 API 或生成失败时使用规则统计文案兜底。
...
| 全量数据 | `api/lastest/all.json` | `type=all`，返回全部榜单、分类、趋势和书籍 |
| 单榜单数据 | `api/lastest/<board>.json` | 返回指定榜单，例如 `api/lastest/female_new.json` |
| 单类型数据 | `api/lastest/<类型>.json` | 兼容旧接口，返回默认榜单下指定类型的数据 |

示例：

```bash
curl https://<你的用户名>.github.io/FanqieRankTracker/api/lastest/all.json
curl https://<你的用户名>.github.io/FanqieRankTracker/api/lastest/female_new.json
